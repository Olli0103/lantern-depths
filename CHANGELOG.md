...
- Common item-relation buttons use the original grammar's `FIND` flags, read live from the VM: **Light with…** uses FLAMEBIT, **Attack with…** WEAPONBIT, **Dig / Unlock / Inflate / Turn with…** TOOLBIT. A leaflet offers Tie/Put in/Put on/Give only; a matchbook gains Light with… once the engine lights it. FLAMEBIT, TOOLBIT and WEAPONBIT were added to `docs/story-symbols.json` after matching object sets on both builds (`tests/relations.test.js`).
- `FIND` flags are inference preferences, not hard parser restrictions. The full parser remains unrestricted. Opening attribution remains in scrollback while the first room description is initially visible.
