...
Accepted direction, 2026-09-09. The environment is the game, not a decorative header above a list of nouns. Remove interface friction, not mystery. Finish connected regions before widening scope. The acceptance criteria below are implemented for the illustrated edition; final release validation is recorded in VALIDATION.md. The delivery log preserves earlier incomplete increments.
...
- [x] Keep the archived story public until rebuilt-story object IDs, flags, globals and saves have a verified build-specific adapter. Do not infer stable symbols from nonunique display names.
...
- [x] Audit scenery, prop positions, scale and source semantics room by room.
- [x] All puzzle-relevant states visible and consistent with the VM, including closed/open, taken/dropped, concealed/revealed, combat and light.
...
- [x] Evaluate an existing lightweight visual placement tool before making a small local coordinate editor. Do not build a general-purpose editor.
...
- [x] Work through Dam/Reservoir, Temple, River, Mine and endgame as connected puzzle regions.
- [x] Improve resolution selectively for puzzle-critical scenes; favour state correctness and coherent object lighting over bulk imagery.
- [x] Discovery-only drawn map, sparse ambient/audio cues, no inferred exits or maze solving.
- [x] Validate a source-built release adapter and explicitly resolve old-save compatibility before changing the public story.
...
## Illustrated edition completion

- All 110 rooms: nonempty authored captions and safe prop surfaces; identical maze/forest scenes retain their ambiguity. Mirror and engraving targets added without revealing paths.
- Targeted hero/state art: altar, iron shaft/chain, lower-shaft basket positions, excavation, Loud Room, broken passage door and final cavern; open treasure containers, ivory torch, open book, silver chalice and lit lamp.
