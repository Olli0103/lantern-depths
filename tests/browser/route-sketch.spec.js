...
test('route sketch has only traversed arrows, keyboard notes, zoom and no game turns',async({page})=>{
 ...
 await page.locator('#map-close').click();await page.locator('#undo').click();await page.locator('#preferences summary').click();await page.locator('#map').click();await expect(page.locator('.journey-node')).toHaveCount(2);await expect(page.locator('.journey-edge')).toHaveCount(1);
});
