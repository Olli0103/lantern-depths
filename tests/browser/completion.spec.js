...
test('corrected state art decodes after restore, hover, mobile selection and final Undo',async({page})=>{
 ...
 const altar=states.find(s=>s.label==='altar');await page.setViewportSize({width:390,height:844});await page.evaluate(s=>localStorage.setItem('lantern-depths.save.v1',JSON.stringify(s)),altar.save);await page.locator('#load').click();await page.locator('#objects [data-select-id="196"]').click();await expect(page.getByRole('button',{name:'Read',exact:true})).toBeVisible();await page.screenshot({path:'test-results/completion-mobile.png',fullPage:true});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
});
