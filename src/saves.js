...
export function capture(engine,history,discovery,storyHash){
  return {format:'lantern-depths',version:2,storyHash,adapter:'zork1-r119-v1',savedAt:new Date().toISOString(),engine:engine.snapshot(),history:structuredClone(history),discovery:structuredClone(discovery.snapshot())};
}
export function decodeSave(raw,ZVM,story,storyHash,futureEntropy){
  ...
  if(save.version!==undefined&&(save.format!=='lantern-depths'||save.version!==2||save.storyHash!==storyHash||save.adapter!=='zork1-r119-v1'))throw Error('This save belongs to a different story or build.');
  ...
  const engine=new Engine(ZVM,story);engine.restore(data,futureEntropy);
