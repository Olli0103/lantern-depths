...
const mounts={
 2:{83:[44,77,8]},129:{26:[43,44,7],196:[56,46,10]},241:{179:[51,76,30]},4:{23:[49,78,19]},
...
export function stageExpedition(engine,layers){
 ...
 for(const layer of layers){
  ...
  if(id===108)point=room===245?[64,78,21]:[49,74,24];
  if(id===203)point=room===245?[45,67,8]:[54,21,8];
 ...
 for(const layer of layers){
  ...
  if(parent===35){const siblings=layers.filter(l=>engine.parent(l.id)===35),i=siblings.indexOf(layer);Object.assign(layer,{x:50+(i-(siblings.length-1)/2)*7,y:46,width:Math.min(10,24/Math.max(1,siblings.length)),placement:'altar'});continue;}
  ...
  const siblings=layers.filter(l=>engine.parent(l.id)===parent),index=siblings.indexOf(layer),cols=Math.max(2,Math.ceil(Math.sqrt(siblings.length)));
