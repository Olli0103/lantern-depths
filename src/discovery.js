...
export function renderDiscovery(discovery,parent,current){
  ...
  const heading=document.createElement('p');heading.textContent='Only places seen and routes travelled. No guessed connections or automatic travel. Indistinguishable mazes and passages must still be navigated by you.';parent.append(heading);
  ...
  for(const room of discovery.rooms){
    const card=document.createElement('section');card.className='discovery-room';
